...
public class FibPair {
    ...
    private static Pair<Integer> nextPairAfter( Pair<Integer> pair) {
        return new Pair<Integer>( // sum of the given pair
								  pair.getFirst() + pair.getSecond()
								  // the former old big number
								  , pair.getFirst()
								  );
    ...
    private static Integer fib( int n) {
		return fibHelper(n).getFirst();
    }
	
	public static Pair<Integer> fibHelper( int n){
		// base cases
		if (n == 0) return new Pair<Integer>(0,0);
		else if( n == 1) return new Pair<Integer>(1,0);
		
		// recursive case(s)
		else{ Pair<Integer> recAbstraction = fibHelper(n-1);
			  return new Pair<Integer>( recAbstraction.getFirst() + recAbstraction.getSecond()
										, recAbstraction.getFirst()
										);
		}
	}
}
